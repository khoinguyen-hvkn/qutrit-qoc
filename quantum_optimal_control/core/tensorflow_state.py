...
class TensorflowState:
        ...
        def matexp_op_grad(uks, H_all, grad):  
            ...
            for ii in range(1,input_num):
                coeff_grad.append(tf.reduce_sum(tf.multiply(grad, tf.matmul(H_all[ii], 
                                                                            matexp, 
                                                                            a_is_sparse=self.sys_para.sparse_H, 
                                                                            b_is_sparse=self.sys_para.sparse_U))))
    ...
    def init_tf_ops_weight(self):
        ...
        print("self.sys_para.ops_len: ", self.sys_para.ops_len)
        print("self.sys_para.ops_max_amp shape: ", np.shape(self.sys_para.ops_max_amp))
        print("self.ops_weight shape: ", np.shape(self.ops_weight))
        print("self.ops_weight: ", self.ops_weight)
    ...
    def init_tf_propagator(self):
        ...
        self.inter_states[0] = tf.matmul(tf_inter_state_op[0], 
                                         self.tf_initial_unitary, 
                                         a_is_sparse=self.sys_para.sparse_U, 
                                         b_is_sparse=self.sys_para.sparse_K)
        
        #subsequent operation layers and intermediate propagators
        for ii in np.arange(1,self.sys_para.steps):
            self.inter_states[ii] = tf.matmul(tf_inter_state_op[ii], 
                                              self.inter_states[ii-1], 
                                              a_is_sparse=self.sys_para.sparse_U, 
                                              b_is_sparse=self.sys_para.sparse_K)
